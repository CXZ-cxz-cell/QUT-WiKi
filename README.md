...
| 内容类型           | 存放目录          |
| ------------------ | ----------------- |
| 校园生活、住宿、餐饮 | `docs/life/`      |
| 学习资源、课程经验   | `docs/study/`     |
| 社团、活动          | `docs/clubs/`     |
| 交通、出行          | `docs/transport/` |
| 使用指南            | `docs/guide/`     |
| 示例                | `docs/examples/`  |
...
- 词与词之间用短横线 `-` 连接
- 示例：`canteen-guide.md`、`library-rules.md`
...
为了让你的文档出现在左侧导航中，请编辑 `docs/.vitepress/config.ts`，在对应分组的 `items` 中追加：

```ts
{ text: '我的文档', link: '/life/canteen-guide' }
```

注意 `link` 不要带 `.md` 后缀，并以 `/` 开头对应 `docs/` 目录。
...
```bash
git add docs/life/canteen-guide.md
